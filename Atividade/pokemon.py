...
def evolucao_anterior(nome):
    ...
    dict = response.json()
    try:
        return dict['evolves_from_species']['name']
    except:
        return None
...
def nivel_do_pokemon(nome, experiencia):
    ...
    if response.status_code != 200:
        raise PokemonNaoExisteException
    dict = response.json()
    url_growth_rate = dict['growth_rate']['url']
    response = requests.get(url_growth_rate)
    dict = response.json()
    for x in dict['levels']:
        if experiencia < x['experience']:
            level = x['level']
            return level - 1
    return 100
...
def cadastrar_treinador(nome):
    response = requests.put(f'{site_treinador}/treinador/{nome}')
...
def cadastrar_pokemon(nome_treinador, apelido_pokemon, tipo_pokemon, experiencia):
    url_pokemon = f'https://pokeapi.co/api/v2/pokemon/{tipo_pokemon.lower()}'
    response1 = requests.get(url_pokemon)
    if response1.status_code != 200:
        raise PokemonNaoExisteException
    dict = response1.json()
    tipo_pokemon = dict['name']
    cad_pokemon = requests.put(f'{site_treinador}/treinador/{nome_treinador}/{apelido_pokemon}',
                               json={'tipo': tipo_pokemon, 'experiencia': experiencia})
    if cad_pokemon.status_code == 404:
        raise TreinadorNaoCadastradoException
    if cad_pokemon.status_code == 409:
...
ou o treinador existe mas o pokemon não. Isso pode ser verificado acessando a resposta.text
...
def ganhar_experiencia(nome_treinador, apelido_pokemon, experiencia):
    ...
    url_treinador = requests.get(
        f'{site_treinador}/treinador/{nome_treinador}')
    response = requests.post(url, json={'experiencia': experiencia})
    if url_treinador.status_code == 404:
        raise TreinadorNaoCadastradoException
...
def localizar_pokemon(nome_treinador, apelido_pokemon):
    ...
    pokemon = Pokemon(nome_treinador, apelido_pokemon, tipo, experiencia, nivel_do_pokemon(
        tipo, experiencia), cor_do_pokemon(tipo), evolucao_anterior(tipo))
    return pokemon
...
def detalhar_treinador(nome_treinador):
    ...
    response = requests.get(url_treinador)
    if response.status_code == 404:
        raise TreinadorNaoCadastradoException
    dict = response.json()
    dic_pokemons = {}
    for x in dict['pokemons']:
        dic_pokemons.update({x: dict['pokemons'][x]['tipo']})
    return dic_pokemons
...
def excluir_treinador(nome_treinador):
    url_treinador = f'{site_treinador}/treinador/{nome_treinador}'
    response = requests.delete(url_treinador)
    if response.status_code == 404:
        raise TreinadorNaoCadastradoException
